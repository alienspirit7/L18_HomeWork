...
print("="*60)
print("LOGISTIC REGRESSION COMPARISON: Manual vs Scikit-Learn")
print("="*60)
...
# Using high max_iter and solver='lbfgs' for convergence
...
print(f"  Coefficients: β0={sklearn_model.intercept_[0]:.6f}, "
      f"β1={sklearn_model.coef_[0][0]:.6f}, β2={sklearn_model.coef_[0][1]:.6f}")
print(f"  Accuracy: {accuracy_sklearn*100:.2f}%")
print(f"  Misclassified points: {misclassified_sklearn}/{len(y)}")
print(f"  Number of iterations: {sklearn_model.n_iter_[0]}")

# ===== LOAD MANUAL IMPLEMENTATION RESULTS =====
print("\n" + "="*60)
print("MANUAL IMPLEMENTATION (from logistic_regression.py)")
print("="*60)
...
# Extract manual coefficients (from last iteration)
beta_manual = manual_history.iloc[-1][['beta0', 'beta1', 'beta2']].values

# Get manual predictions and probabilities
y_prob_manual = manual_results['sigma(x1,x2)'].values[:-1]  # Exclude last row (average)
...
print(f"  Coefficients: β0={beta_manual[0]:.6f}, "
      f"β1={beta_manual[1]:.6f}, β2={beta_manual[2]:.6f}")
print(f"  Accuracy: {accuracy_manual*100:.2f}%")
print(f"  Misclassified points: {misclassified_manual}/{len(y)}")
print(f"  Number of iterations: {len(manual_history)}")
...
comparison_data = {
    ...
    'Manual': [
        ...
        f"{beta_manual[1]:.6f}",
        f"{beta_manual[2]:.6f}",
        f"{accuracy_manual*100:.2f}",
        f"{misclassified_manual}",
        f"{len(manual_history)}"
    ],
    'Scikit-Learn': [
        f"{sklearn_model.intercept_[0]:.6f}",
        f"{sklearn_model.coef_[0][0]:.6f}",
        f"{sklearn_model.coef_[0][1]:.6f}",
        f"{accuracy_sklearn*100:.2f}",
        f"{misclassified_sklearn}",
        f"{sklearn_model.n_iter_[0]}"
    ],
    'Difference': [
        f"{abs(beta_manual[0] - sklearn_model.intercept_[0]):.6f}",
...
# Subplot 2: Probability Distribution Comparison
ax2 = fig.add_subplot(132)

# Scatter plot comparing probabilities
scatter = ax2.scatter(y_prob_manual, y_prob_sklearn,
                     c=y, cmap='RdYlBu', alpha=0.6, s=20, edgecolors='black', linewidth=0.5)
...
ax2.text(0.05, 0.95, f'Correlation: {correlation:.6f}',
...
bars1 = ax3.bar(x_pos - width/2, manual_metrics, width, label='Manual', color='steelblue', alpha=0.8)
bars2 = ax3.bar(x_pos + width/2, sklearn_metrics, width, label='Scikit-Learn', color='darkorange', alpha=0.8)
...
# ===== 3D VISUALIZATION: SIDE-BY-SIDE DECISION BOUNDARIES =====
...
# Manual implementation sigmoid
def sigmoid(z):
    return 1 / (1 + np.exp(-z))

# Manual decision surface
...
ax1_3d.scatter(X[group0, 0], X[group0, 1], y_prob_manual[group0],
              c='blue', marker='o', label='Class 0', alpha=0.6, s=20)
ax1_3d.scatter(X[group1, 0], X[group1, 1], y_prob_manual[group1],
              c='red', marker='^', label='Class 1', alpha=0.6, s=20)

# Plot decision surface
surf1 = ax1_3d.plot_surface(xx1, xx2, Z_manual, alpha=0.3, cmap='viridis', edgecolor='none')
...
ax2_3d.scatter(X[group0, 0], X[group0, 1], y_prob_sklearn[group0],
              c='blue', marker='o', label='Class 0', alpha=0.6, s=20)
ax2_3d.scatter(X[group1, 0], X[group1, 1], y_prob_sklearn[group1],
              c='red', marker='^', label='Class 1', alpha=0.6, s=20)

surf2 = ax2_3d.plot_surface(xx1, xx2, Z_sklearn, alpha=0.3, cmap='viridis', edgecolor='none')
...
print(f"  3. {decision_surface_path}")
